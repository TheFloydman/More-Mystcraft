...
public class EntityMaintainerSuit extends Entity {
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager
			.<Integer>createKey(EntityMaintainerSuit.class, DataSerializers.VARINT);
	...
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager
			.<Float>createKey(EntityMaintainerSuit.class, DataSerializers.FLOAT);
	...
	private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[] {
			EntityDataManager.createKey(EntityMaintainerSuit.class, DataSerializers.BOOLEAN),
			EntityDataManager.createKey(EntityMaintainerSuit.class, DataSerializers.BOOLEAN) };
	...
	private EntityMaintainerSuit.Status status;
	private EntityMaintainerSuit.Status previousStatus;
	private double lastYd;

	public EntityMaintainerSuit(World worldIn) {
		super(worldIn);
	...
	public EntityMaintainerSuit(World worldIn, double x, double y, double z) {
		this(worldIn);
	...
	protected void entityInit() {
		...
		this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityMaintainerSuit.Type.OAK.ordinal()));
	...
	public void applyEntityCollision(Entity entityIn) {
		if (entityIn instanceof EntityMaintainerSuit) {
	...
	public void onUpdate() {
		...
		if (this.status != EntityMaintainerSuit.Status.UNDER_WATER
				&& this.status != EntityMaintainerSuit.Status.UNDER_FLOWING_WATER) {
	...
	public float getRowingTime(int side, float limbSwing) {
		return this.getPaddleState(side)
				? (float) MathHelper.clampedLerp((double) this.paddlePositions[side] - 0.39269909262657166D,
	 ...
	 */
	private EntityMaintainerSuit.Status getBoatStatus() {
		EntityMaintainerSuit.Status EntityMaintainerSuit$status = this.getUnderwaterStatus();

		if (EntityMaintainerSuit$status != null) {
			this.waterLevel = this.getEntityBoundingBox().maxY;
			return EntityMaintainerSuit$status;
		} else if (this.checkInWater()) {
			return EntityMaintainerSuit.Status.IN_WATER;
		} else {
			float f = this.getBoatGlide();

			if (f > 0.0F) {
				this.boatGlide = f;
				return EntityMaintainerSuit.Status.ON_LAND;
			} else {
				return EntityMaintainerSuit.Status.IN_AIR;
			}
		}
	}
	 ...
	 */
	@Nullable
	private EntityMaintainerSuit.Status getUnderwaterStatus() {
		AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
					...
					for (int i2 = i1; i2 < j1; ++i2) {
						...
						if (iblockstate.getMaterial() == Material.WATER && d0 < (double) BlockLiquid
							...
							if (((Integer) iblockstate.getValue(BlockLiquid.LEVEL)).intValue() != 0) {
								EntityMaintainerSuit.Status EntityMaintainerSuit$status = EntityMaintainerSuit.Status.UNDER_FLOWING_WATER;
								return EntityMaintainerSuit$status;
							}
		...
		return flag ? EntityMaintainerSuit.Status.UNDER_WATER : null;
	}
	...
	private void updateMotion() {
		...
		if (this.previousStatus == EntityMaintainerSuit.Status.IN_AIR
				&& this.status != EntityMaintainerSuit.Status.IN_AIR
				&& this.status != EntityMaintainerSuit.Status.ON_LAND) {
			this.waterLevel = this.getEntityBoundingBox().minY + (double) this.height;
			this.setPosition(this.posX, (double) (this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
			this.motionY = 0.0D;
			this.lastYd = 0.0D;
			this.status = EntityMaintainerSuit.Status.IN_WATER;
		} else {
			if (this.status == EntityMaintainerSuit.Status.IN_WATER) {
				d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double) this.height;
				this.momentum = 0.9F;
			} else if (this.status == EntityMaintainerSuit.Status.UNDER_FLOWING_WATER) {
				d1 = -7.0E-4D;
				this.momentum = 0.9F;
			} else if (this.status == EntityMaintainerSuit.Status.UNDER_WATER) {
				d2 = 0.009999999776482582D;
				this.momentum = 0.45F;
			} else if (this.status == EntityMaintainerSuit.Status.IN_AIR) {
				this.momentum = 0.9F;
			} else if (this.status == EntityMaintainerSuit.Status.ON_LAND) {
	...
	protected void readEntityFromNBT(NBTTagCompound compound) {
		if (compound.hasKey("Type", 8)) {
			this.setBoatType(EntityMaintainerSuit.Type.getTypeFromString(compound.getString("Type")));
		}
	}
		...
		if (!this.isRiding()) {
			if (onGroundIn) {
				if (this.fallDistance > 3.0F) {
					if (this.status != EntityMaintainerSuit.Status.ON_LAND) {
	...
	public void setBoatType(EntityMaintainerSuit.Type boatType) {
		this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
	}

	public EntityMaintainerSuit.Type getBoatType() {
		return EntityMaintainerSuit.Type.byId(((Integer) this.dataManager.get(BOAT_TYPE)).intValue());
	}
	...
	public static enum Type {
		 ...
		 */
		public static EntityMaintainerSuit.Type byId(int id) {
		...
		public static EntityMaintainerSuit.Type getTypeFromString(String nameIn) {
