...
public class SymbolTintedSun extends MoreMystcraftSymbolBase {
    ...
    public void registerLogic(final AgeDirector controller, final long seed) {
        final Number period = controller.popModifier(ModifierUtils.FACTOR).asNumber();
        final Number angle = controller.popModifier(ModifierUtils.ANGLE).asNumber();
        final Number phase = controller.popModifier(ModifierUtils.PHASE).asNumber();
        final ColorGradient sunset = controller.popModifier(ModifierUtils.SUNSET).asGradient();
