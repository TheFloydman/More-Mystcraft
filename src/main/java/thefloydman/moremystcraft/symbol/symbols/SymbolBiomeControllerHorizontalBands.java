...
public class SymbolBiomeControllerHorizontalBands extends MoreMystcraftSymbolBase {

	public SymbolBiomeControllerHorizontalBands(final ResourceLocation identifier) {
		super(identifier);
	}
	...
	private class BiomeController implements IBiomeController {
		...
		public Biome getBiomeAtCoords(final int i, final int j) {
			...
			int bandNumber = (int) (verticalDistance / this.size);
			while (bandNumber >= this.biomes.size()) {
				bandNumber -= this.biomes.size();
			}
