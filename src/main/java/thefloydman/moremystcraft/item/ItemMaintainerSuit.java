...
public class ItemMaintainerSuit extends Item {
	private final EntityMaintainerSuit.Type type;

	public ItemMaintainerSuit(EntityMaintainerSuit.Type typeIn) {
		this.type = typeIn;
		this.maxStackSize = 1;
		this.setCreativeTab((CreativeTabs) MystcraftCommonProxy.tabMystCommon);
		this.setUnlocalizedName("moremystcraft.maintainer_suit");
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (raytraceresult == null) {
			...
			if (flag) {
				...
				EntityMaintainerSuit entitySuit = new EntityMaintainerSuit(worldIn, raytraceresult.hitVec.x,
						flag1 ? raytraceresult.hitVec.y - 0.12D : raytraceresult.hitVec.y, raytraceresult.hitVec.z);
				entitySuit.setBoatType(this.type);
				entitySuit.rotationYaw = playerIn.rotationYaw;

				if (!worldIn.getCollisionBoxes(entitySuit, entitySuit.getEntityBoundingBox().grow(-0.1D)).isEmpty()) {
					return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
				} else {
					if (!worldIn.isRemote) {
						worldIn.spawnEntity(entitySuit);
					}
